...
async function getAllProducts(reqQuery){
    let {category,color,sizes,minPrice,minDiscount,sort,stock,pageNumber,pageSize}=reqQuery;
    ...
    if(color){
        const colorList=new Set(color.split(",").map(color=>color.trim().toLowercase()));
        const colorRegex=colorSet.size>0?new RegExp([...colorSet].join("|"),"1"):null;
